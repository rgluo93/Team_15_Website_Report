...
UAT was conducted in collaboration with Dignitas, our project partner. Following a demo of the new features, the client organised testing sessions in which teachers were given the opportunity to explore the developer environment hands-on. Each session followed a structured format: participants completed a series of guided steps to trial a given feature, then filled out a short questionnaire. Responses were recorded on a 1–5 Likert scale (1 = Strongly Disagree, 5 = Strongly Agree), with participants also providing written feedback to explain their ratings. The client has since returned aggregated results from these sessions and gave us a feedback summary. Note that the data does not include the total number of teacher respondents.
...
## Feedback Conclusion 
Overall, the user acceptance testing revealed that the new AI features were generally well-received by teachers, with positive feedback on the UI and the relevance of the content. However, there were some areas for improvement, particularly in terms of the accuracy of the education officer agent and the speech to text feature. These issues are largely due to limitations in the underlying machine learning models and can be addressed in future iterations of the project. During the handover meeting with Dignitas and Tech To The Rescue, our clients were extremely satisfied with the results of our project and complimented our achievements. 