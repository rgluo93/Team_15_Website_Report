...
- **Hybrid search** (vector + keyword) to improve retrieval accuracy
- **Re-ranking models** to select the most relevant chunks before passing them to the LLM
- **Automatic document updates** so new LeadNow content is indexed without manual intervention

This would reduce hallucinations and produce more accurate, grounded responses.

## 2. Personalised Learning Path Recommendations

Rather than generating generic study plans, the system could:

- Analyse teacher performance across modules
- Detect knowledge gaps and weak areas
- Automatically recommend specific modules or exercises based on individual progress

For example: *"Teachers struggling with classroom management should complete modules 20.2.0 and 20.3.0 next."*

This would transform the platform into a true adaptive learning system.

## 3. Voice-Based Chatbot Interaction

Currently, speech-to-text is only used for recording answers. A fuller voice experience could include:

- Full voice conversation with the chatbot
...
## 4. Offline Mode for Low Connectivity
...
## 5. Analytics Dashboard for Education Officers

The education officer agent currently responds to natural language prompts. A future extension could add:
...
## 6. Improved AI Feedback Evaluation

Currently the AI generates open-ended feedback for scenario-based answers. This could be extended with:

- Rubric-based grading aligned to LeadNow standards
- Comparison against expert teacher reference responses
- Confidence scores to indicate feedback reliability

This would produce more consistent and trustworthy automated marking.

## 7. Multi-language Support Beyond Swahili
...
This would make teacher training more inclusive across diverse communities.

## 8. Smarter Agent Planning

The education officer agent could become significantly more capable with:

- Additional tools (calendar scheduling, automated report generation)
- Multi-agent collaboration for complex workflows
- Automatic school visit scheduling based on teacher performance data

For example: *"Schedule visits to the three lowest-performing schools next week."*

## 9. Model Fine-tuning on Teacher Data

Rather than relying on general-purpose language models, domain-specific fine-tuning could be applied using:

- LeadNow curriculum content
- Historical teacher responses and feedback

This would produce more contextually appropriate coaching and more accurate content-specific answers.

## 10. Impact Evaluation and A/B Testing
